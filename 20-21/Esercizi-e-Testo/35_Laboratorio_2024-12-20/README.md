# Laboratorio del 20/12/2024, ore 12:30 - 14:30, aula Delta
...
1. Si completi la seguente classe Board, che implementa una tavola bidimensionale di cellule,
rappresentate come booleani. Il valore true indica che la cellula è viva, mentre il valore false
indica che essa è morta. Il metodo toString() (già scritto) riporta le cellule vive con un
asterisco e quelle morte con uno spazio.
...
La classe Board definisce un metodo privato next() che modifica le cellule facendole transitare dalla generazione corrente a quella successiva: il modo in cui si calcola la generazione
successiva da quella corrente è specificato tramite un oggetto che implementa la seguente
interfaccia:
...
2. Si crei una classe di prova MainAtLeast3 il cui main crea una Board di dimensione 40x20 con
esattamente 100 cellule vive posizionate a caso, quindi chiama il metodo play su tale Board,
con un processore che rende una cellula viva alla prossima generazione, se e solo se nella
generazione corrente essa ha almeno 3 cellule adiacenti vive. Suggerimento: definite una
classe che implementa NextAliveProcessor e che riceve la Board tramite il suo costruttore.
In questo modo il processore può controllare lo stato delle cellule adiacenti.

3. Si crei una classe di prova MainGameOfLife identica a MainAtLeast3, ma che usa un processore che rende una cellula viva alla prossima generazione basandosi sulle quattro regole del
gioco della vita.

4. Si crei una classe di prova MainConstant identica a MainAtLeast3, ma che usa un processore
che rende una cellula viva alla prossima generazione se e solo se essa è viva nella generazione
corrente (quindi il gioco non evolverà).

5. Si reimplementino i punti 2-3-4 usando delle lambda espressioni come processori.

## Diagrammi e Schemi

### Vicini

![Vicini](immagini/vicini.png)

## Materiali Didattici ed Esercizi

### Soluzione

- 💻 [**`Board.java`**](soluzione/Board.java)
- 💻 [**`MainAtLeast3.java`**](soluzione/MainAtLeast3.java)
- 💻 [**`MainAtLeast3Lambda.java`**](soluzione/MainAtLeast3Lambda.java)
- 💻 [**`MainConstant.java`**](soluzione/MainConstant.java)
- 💻 [**`MainConstantLambda.java`**](soluzione/MainConstantLambda.java)
- 💻 [**`MainGameOfLife.java`**](soluzione/MainGameOfLife.java)
- 💻 [**`MainGameOfLifeLambda.java`**](soluzione/MainGameOfLifeLambda.java)
- 💻 [**`NextAliveProcessor.java`**](soluzione/NextAliveProcessor.java)

## Collegamenti e Riferimenti Esterni

- [Descrizione del Gioco della Vita (Wikipedia)](https://it.wikipedia.org/wiki/Gioco_della_vita#Descrizione)
- [Link alla registrazione della soluzione (Panopto)](https://univr.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=0a0ba1b6-da52-4a8f-b048-b24f0085254a)
...
<nav class="lesson-nav" style="position: fixed; top: 24px; right: 20px; width: 125px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Noto Sans', Helvetica, Arial, sans-serif; font-size: 11px; line-height: 1.45; border-left: 1px solid #d0d7de; padding-left: 10px; z-index: 100;">
  ...
  <div style="display: flex; gap: 8px; margin-bottom: 8px; font-size: 10px; color: #656d76;">
    <a href="../34_Lezione_2024-12-19/README.md" style="color: #0969da; text-decoration: none;" title="Lezione del 19/12/2024, ore 16:30 - 18:30, aula Magna">← Prec</a>
    <span style="color: #d0d7de;">|</span>
    <a href="../36_Laboratorio_2025-01-10/README.md" style="color: #0969da; text-decoration: none;" title="Laboratorio del 10/1/2025, ore 12:30 - 14:30, aula Delta">Succ →</a>
  </div>
  <ul style="list-style: none; margin: 0; padding: 0; max-height: calc(100vh - 80px); overflow-y: auto; scrollbar-width: thin;">
    ...
    <li style="margin: 2px 0; font-weight: 600; color: #0969da; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="Laboratorio del 20/12/2024, ore 12:30 - 14:30, aula Delta">35. Lab 20/12</li>
